                ...
                @else
                    <div class="relative overflow-x-auto shadow-md sm:rounded-lg mb-5">
                        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    <th scope="row" class="px-6 py-3">ID</th>
                                    <td class="px-6 py-3">{{ $task->id }}</td>
                                </tr>
                                <tr
                                    class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                    <th scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">Submission Type</th>
                                    <td class="px-6 py-3">{{ $task->submission_type }}</td>
                                </tr>
                                <tr>
                                    <th scope="row" class="px-6 py-3">Due Date</th>
                                    <td class="px-6 py-3">{{ $task->due_date }}</td>
                                </tr>
                                <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                    <th scope="row" class="px-6 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">Submission</th>
                                    <td class="px-6 py-3">
                                        @if ($task->submission)
                                            {{ $task->submission->online_text }}
                                        @else
